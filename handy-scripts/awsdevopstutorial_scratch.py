...
base_url = 'https://zzz00000.live.dynatrace.com'
problem_id = '-3920663950601979683'
individual_url = '$base_url/api/v1/problem/details/$uniqueid'


def get_events(base_url, f_headers, entityId='', eventType=''):
    # Notification URL
    uri = '/api/v1/events'
    events_url = urllib.parse.urljoin(base_url, uri)

    final_events_url = events_url

    if entityId != '' or eventType != '':
        final_events_url =  final_events_url + '?'

    if entityId != '' and eventType != '':
        final_events_url = final_events_url + 'entityId=' + entityId
        if eventType != '':
            final_events_url = final_events_url + '&eventType=' + eventType

    if entityId == '' and eventType != '':
        final_events_url = final_events_url + 'eventType=' + eventType

    print(final_events_url)

    # Submit Comment
    events_response = {}
    requests_response = False
    requests_response = try_request(
        f_url=final_events_url,
        f_headers=f_headers,
        log_category='GetEvents',
        error_msg='GetEvents - Unable to retrieve events',
        log_key='problem_id',
        log_value=problem_id,
        f_dict=events_response)
...
events_list = get_events(base_url=base_url,
                         entityId='SERVICE-9F279FD3D7DAEF8E',
...
individual_url = build_individual_url(
                    base_url=base_url,
